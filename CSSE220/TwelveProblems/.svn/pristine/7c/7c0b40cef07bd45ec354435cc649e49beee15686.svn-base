...
public class TwelveProblems {
	...
	public static boolean endsWithUpperCaseLetter(String input) {
		...
		char lastcase = input.charAt(input.length()-1);
		if(Character.isUpperCase(lastcase)){
			return true;
		}
		return false;	
	}
	...
	public static int[] powersOfTwo(int maxExponent) {
		...
		if(maxExponent>=0){
			int[] newarray = new int[maxExponent+1];
