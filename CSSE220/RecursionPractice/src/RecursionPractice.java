...
public class RecursionPractice {
	 ...
	 */
	public static int arrayListSummation(List<Integer> list) {
		if (list.size() == 1) {
			return list.get(0);
		}
		int first = list.get(0);
		list.remove(0);
		return first + arrayListSummation(list);
	}
	 ...
	 * 
	 */
	public static boolean bracket(char a, char b) {
		if (a == '(' && b == ')') {
			return true;
		}
		if (a == ')' && b == '(') {
	...
	public static boolean bracketMatch(String inputString) {
		...
		if (inputString.length() == 2) {
			if (bracket(inputString.charAt(0), inputString.charAt(1))) {
				return true;
			}
		}
	...
	public static boolean evenPowerOfThree(int inputValue) {
		...
		if ((inputValue / 9) % 1 == 0 && inputValue != 0) {
	...
	public static boolean numbersGame(int startingNumber) {
		...
		if (startingNumber % 3 == 0 && startingNumber > 7) {
