...
def problem3(line, window, m, color):
    ...
    x = line.get_midpoint().x + LENGTH / 2
    y = line.get_midpoint().y
    circenter = rg.Point(x, y)
    x1 = line.get_midpoint().x + LENGTH * 3 / 2
    y1 = line.get_midpoint().y
    sqcenter = rg.Point(x1, y1)

    for _ in range(m):
        circle = rg.Circle(circenter, LENGTH / 2)
        circle.fill_color = color
        circle.attach_to(window)
        circenter.x = circenter.x + LENGTH * 2

    for _ in range(m):
        square = rg.Square(sqcenter, LENGTH)
        square.fill_color = line.color
        square.attach_to(window)
        sqcenter.x = sqcenter.x + LENGTH * 2
