...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Qishun Yu.  March 2016.
"""
...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        i = circle.center.x + k * circle.radius * 2
        j = circle.center.y + k * circle.radius * 2
        point = rg.Point(i, j)
...
def test_draw_circles_from_rectangle():
    ...
    # Test 1:
    rectangle = rg.Rectangle(rg.Point(100, 200), rg.Point(150, 225))
    draw_circles_from_rectangle(5, 4, rectangle, window1)

    # Test 2:
    rectangle = rg.Rectangle(rg.Point(300, 100), rg.Point(350, 200))
    ...
    window2 = rg.RoseWindow(650, 350, title)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    center1.x = center1.x - rectangle.get_width() / 2 - h / 2

    center2 = rectangle.get_center()
    center2.x = center2.x + rectangle.get_width() / 2 + w / 2

    for _ in range(m):
        circle = rg.Circle(center1, h / 2)
        circle.fill_color = rectangle.outline_color
        circle.attach_to(window)

        center1.x = center1.x - h

    for _ in range(n):
        circle = rg.Circle(center2, w / 2)
        circle.fill_color = rectangle.outline_color
        circle.attach_to(window)

        center2.x = center2.x + w
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    xd = rectangle1.get_width() / 2
    yd = rectangle1.get_height() / 2
