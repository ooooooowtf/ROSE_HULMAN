...
def my_frame(root, dc):
    ...
    # Bio button
    bio_label = ttk.Label(frame, text='Create a ridiculous bio')
    bio_label.grid()
    bio_button = ttk.Button(frame, text='Print')
    bio_button['command'] = lambda: create_bio()
    bio_button.grid()
...
def create_bio():
    ...
    """
    adjective = ['amateur', 'hardcore', 'passionate', 'proud', 'extreme',
                 'ceaseless', 'superb', 'secret', 'slight', 'curious',
                 'eccentric', 'zany']
    noun_1 = ['Twitter', 'TV', 'movie', 'Internet', 'food', 'video game',
              'basketball', 'baseball', 'football', 'soccer', 'sleep',
              'programming', 'rock climbing', 'rugby', 'running', 'chess',
              'laser tag', 'cat', 'dog', 'pet', 'heavy metal', 'bagpipe',
              'EDM', 'jazz', 'opera', 'tattoo', 'art', 'ice cream', 'pizza',
              'space', 'tuba']
    noun_2 = ['enthusaist', 'junkie', 'scholar', 'troublemaker', 'specialist',
              'fan', 'inventor', 'geek', 'guru', 'leader',
              'master', 'specialist', 'addict', 'extraordinaire', 'hater']
    string = ''
    for _ in range(4):
        part = noun_1[random.randint(0, len(noun_1) - 1)] + ' ' + noun_2[random.randint(0, len(noun_2) - 1)] + '.'
        if random.randint(0, 1) == 1:
            part = adjective[random.randint(0, len(adjective) - 1)] + ' ' + part
