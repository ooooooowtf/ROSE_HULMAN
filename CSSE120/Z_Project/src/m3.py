...
def my_frame(root, dc):
    ...
    # extra feature of dancing
    dancing = ttk.Button(frame1, text='dancing')
    dancing['command'] = (lambda:
                               dancing1(dc))
    dancing.grid()
    return frame1
...
def readreflectance(dc):
    ...
    while True:
        ...
        print('')
        print('')
        dc.angrybot.motor_controller.drive_pwm(40, 40)
...
def followblack(dc):
    ...
    while True:
        ...
        if rs < threshold:
            ...
            dc.angrybot.motor_controller.drive_pwm(Speed, Speed + error)
        if ls < threshold:
            lsum = 0
            difl = threshold - ls
            ddifl = 0
            ...
            dc.angrybot.motor_controller.drive_pwm(Speed + error, Speed)
...
def showangry(dc):
    ...
    # check if the bot bump into a wall, then show angry
