...
def draw_L(window, circle, r, c):
    ...
    for _ in range(r):
        for _ in range(3):
            point = circle.center
            point.x = circle.center.x + circle.radius * 2
            newcircle = rg.Circle(point, circle.radius)
            newcircle.fill_color = circle.fill_color
            newcircle.attach_to(window)
            window.render(0.1)
        point.x = a
        point.y = circle.center.y + circle.radius * 2
    for _ in range(3):
        for _ in range(3 + c):
            point = circle.center
            point.x = circle.center.x + circle.radius * 2
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for k in range(n):
        ...
        point1.x = rectangle.corner_1.x
        point2.x = rectangle.corner_2.x
