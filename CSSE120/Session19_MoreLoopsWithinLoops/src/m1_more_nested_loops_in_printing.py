...
def triangle_right_justified(r):
    ...
    for k in range(r):
        number = 0
        for j in range(k + 1):
            number = number + (j + 1) * 10 ** j
        print(' ' * (r - k - 1) + str(number))
...
def vee(r):
    ...
    for k in range(r):
        ...
        for j in range(r - k):
            number = number + (j + 1) * 10 ** (r - k - j - 1)
            numbe = numbe + (j + 1) * 10 ** j

        print(' ' * (k) + str(number) + '-' + str(numbe))
