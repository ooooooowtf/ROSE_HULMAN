...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Qishun Yu.  April 2016.
...
def test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  200 slices, thin (thickness=2) black lines.'
    ...
    pizza(window, circle, 200, 'black', 2)
    window.close_on_mouse_click()
...
def test_polygon():
    ...
    # Test 3:
    title = 'POLYGON test 3: 10 segments, with thick yellow lines'
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 200)
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'yellow', 6)
    window.continue_on_mouse_click()
...
def test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  51 green lines' + \
        ' on red circle, hops = 25.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'red'
    fancy_polygon(window, circle, 20, 7, 'green', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines,
    ...
    for k in range(len(points)):
        linenum = (k + hops_to_next_point) % (number_of_lines)
        line = rg.Line(points[k], points[linenum])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
