...
class Box(object):
    ...
    def shrink(self, new_volume):
        ...
        numbertoadd = min(new_volume, len(new))

        for k in range(numbertoadd):
            self.contents = self.contents + new[k]

        s = ''
        for k in range(new_volume, len(new)):
            s = s + new[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        n = 0
        if len(self.contents) * 2 >= self.volume:
            n = n + len(self.contents) * 2 - self.volume
        self.double()
        if len(self.contents) >= new_volume:
            n = len(self.contents) - new_volume + n
        new = ''
        numbertoadd = min(len(self.contents), new_volume)
        for k in range(numbertoadd):
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.origincon = self.origincon + [self.contents]
